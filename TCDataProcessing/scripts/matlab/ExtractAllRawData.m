%throw(MException('PasstimesFile:noCorrespondingMissionSensor',                             ...
...
%ScriptHeaderFilename = 'SHEM_8_10_Helen_ScriptHeader';
% ==================================================
% ==================================================
% ==================================================
% ==================================================
...
for iFile = 1:length(passtimes_files)
    ...
    try
        ...
        if (isempty(processingExceptions))
            processingExceptions = MException('failedMissionSensor:DataExtraction',             ...
                                              'Failed to extract Mission(%s)-Sensor(%s) data!', ...
                                              mission, sensor);
        end
        processingExceptions = addCause(processingExceptions, ME);
    end
end % End: For (passtimes_file in passtimes_files)

if (~isempty(processingExceptions))
    warnStr = processingExceptions.message;
    if (~isempty(processingExceptions.cause))
        warnStr = sprintf('WARNING: %s\n\nCaused by:\n', warnStr);
        for iCause = 1:length(processingExceptions.cause)
            eCause = processingExceptions.cause{iCause};
            warnStr = sprintf('%s\t%d) %s\n', warnStr, iCause, eCause.message);
            %warnStr = sprintf('%s\t%d)\n', warnStr, iCause,);
        end
    end
    %throw(processingExceptions);
    fprintf(warnStr);
end
