function = CyclonePlot_Density(storm_trackfile__,storm_wwlln_locations__,output_path__,storm_name__,output_name_pattern__)
    clear
    ...
    for day_year = day_year_min:day_year_max %loop over days of interest
        ...
        ch1=colorbar;
        % Graph string
        GraphAxisString = ['log_{10}(strokes / ', num2str(GridSize^2),' km^2 / day)'];
        ...
        title([storm_name__, ' ', datestr(day_year, 'mm/dd/yyyy'),' (Courtesy of WWLLN/UW/NWRA/DigiPen)'])
        %filenameBig = [StormFilenamePrefix,num2str(year),'_',month,'_',day, '_density_plot'];
        filenameBig=sprintf(output_name_pattern__, [num2str(year), '_', month, '_', day]);
